...
export default function VoiceNoteBubble({ text, duration = 5, isUser = true }) {
      ...
      timerRef.current = setInterval(() => {
        setElapsedTime((prev) => {
          ...
          if (next >= duration) {
            ...
            setIsPlaying(false);
            return 0; // Reset to 0 when finished
          }
          return next;
        });
      }, intervalMs);
  ...
  // Determine which bars are "played"
  ...
  const playButtonIconColor = isUser ? C.white : C.accent;
